...
# filename = "7002_08320801862_17-Jun-26-18-11-32.wav16"

# # Split into name and extension
# name, ext = os.path.splitext(filename)

# # Add "-IN" before extension
# new_filename = f"{name}-IN{ext}"

# print("Old:", filename)
# print("New:", new_filename)

# to create the rx nd tx path 
    ...
    for excel_row in ws.iter_rows(min_row=2):
        ...
        for field, idx in header_index.items():
            ...
            if field == "recording":
                ...
                # http://192.168.20.49/Recordings/31Mar2026/4021_08530245867_31-Mar-26-10-20-01.WAV'    
                # record["recording_hyperlink"] = cell.hyperlink.target if cell.hyperlink else None
            else:
...
def process_call(conn, call_row):
    ...
    conn.commit()
    try:
        slm_result = call_slm(conversation_text, utterances)# we dont require the uternace for analysis
    except Exception as e:
        conn.execute(
            "UPDATE calls SET status='failed', error_message=? WHERE id=?",
    ...
    if len(param_data) == 0:
        # print("**"*12)
        print("param_data is empty, sending it again for analysis")
        try:
        ...
        analysis = slm_result.get("analysis", slm_result)
        param_data = analysis.get("parameters", {})
    # if the param_dasta is empty then we will send it again for analysis 
    # print(param_data)
    ...
    print("*param_id_by_key***"*12)
    # print(param_id_by_key)
    scores = []
